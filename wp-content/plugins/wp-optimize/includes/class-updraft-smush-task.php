...
abstract class Updraft_Smush_Task extends Updraft_Task_1_1 {
	...
	public function complete() {
		...
		if (0 == $original_size) {
			$info = sprintf(__("The file was compressed to %s using WP-Optimize", 'wp-optimize'), wpo_format_filesize(filesize($file_path)));
		} else {
			$saved = round((($original_size - filesize($file_path)) / $original_size * 100), 2);
			$info = sprintf(__("The file was compressed from %s to %s saving %s percent using WP-Optimize", 'wp-optimize'), wpo_format_filesize($original_size), wpo_format_filesize(filesize($file_path)), $saved);
		}
